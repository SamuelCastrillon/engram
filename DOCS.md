...
| `ENGRAM_CLOUD_TOKEN_PEPPER`     | Dedicated secret used to hash managed cloud tokens created by `engram cloud bootstrap admin` (and, once server-side managed-token wiring lands, the admin API/dashboard). Distinct from `ENGRAM_JWT_SECRET` on purpose — see [Managed users, tokens, and CLI bootstrap](#managed-users-tokens-and-cli-bootstrap-preview). | (unset)              |
...
- `engram cloud bootstrap admin --username <name> [--email <email>] [--grant-project <project>]... [--issue-token [name]]` — create the first managed admin (see [Managed users, tokens, and CLI bootstrap](#managed-users-tokens-and-cli-bootstrap-preview))
...
| `ENGRAM_CLOUD_TOKEN_PEPPER`     | no (required for `--issue-token`) | Dedicated managed-token hashing secret used by `engram cloud bootstrap admin --issue-token`. Must differ from `ENGRAM_JWT_SECRET`. |

### Managed users, tokens, and CLI bootstrap (preview)
...
**Current limitation (read before relying on this for authentication):** `engram cloud serve` does not yet wire managed-token authentication into its running request path. Today the server's actual authentication for `/sync/*`, `/admin/*`, and dashboard login still resolves only `ENGRAM_CLOUD_TOKEN` (legacy sync) and `ENGRAM_CLOUD_ADMIN` (legacy dashboard bootstrap/admin credential) — the `cloud_principals` storage layer, admin API, and managed-token hasher exist and are covered by tests, but production wiring for managed-token bearer authentication is a separate, not-yet-shipped follow-up. In practice this means:

- `engram cloud bootstrap admin` durably creates and audits the managed admin/token/grant rows described above, ready for when server wiring lands.
- Until that wiring ships, continue using `ENGRAM_CLOUD_TOKEN` for sync clients and `ENGRAM_CLOUD_ADMIN` for dashboard access — a token issued by `--issue-token` cannot yet authenticate a `/sync/*`, `/admin/*`, or dashboard-login request.
- No rollback action is required to keep using legacy credentials: legacy `ENGRAM_CLOUD_TOKEN` / `ENGRAM_CLOUD_ADMIN` behavior is unchanged and remains the supported authentication path for this release.
